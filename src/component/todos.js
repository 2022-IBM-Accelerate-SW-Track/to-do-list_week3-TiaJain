...
const Todos = ({ todos, deleteTodo }) => {
  const todoList = todos.length ? (
    todos.map((todo) => {
      let color = "#ffffffff";
      console.log("right before the date comparison if statement")
      if (new Date(todo.due) < (new Date())) {
        console.log("inside the date comparison if statement")
        color = "#ADD8E6";
      }
      console.log("right after the date comparison if statement")
      return (
        <Grid key={todo.id}>
